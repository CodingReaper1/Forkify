...
// Promises are just like lottary tickets which either get fulfuilled or rejected used for  asichronous javascript you dont have to wait for outcome for code to keep executing

// Asunchronous javascript is all about web apis, event loop and callback queue. for example if we use photo and eventListener("load") on it. photo is now stored in web api waiting to load while code in normally executed in call stack. after loading photo is done code in eventListener(or any callbackfunction) moves in callback queue and EVENT LOOP moves code in callstack after callstack was empty and this code was first in callback queue, how ever fetch() and .then() work little different here code in .then() is put in microtasks queue instead of callback queue and microtasks queue does have advantage over callback queue(microtasks queue is executed before callstack)

// Imports are not copy of exports they are like live connection so they are pointing to exports

// Parcel codes : npm i, npm i parcel --save-dev, npm run start/build
...
const whereAmI = function ([lat, lng]) {
  ...
  fetch(
    `https://geocode.xyz/${lat},${lng}?geoit=json&auth=119918816779022e15946471x111718`
  )
    ...
    .then(data => {
      if (data.error) {
        ...
        renderError(
          `Something went wrong 🤬🤬 geocode api run out of credits. Try again!`
        );
      }
...
const whereAmI2 = async function () {
  try {
    // Geolocation
    const pos = await getPoisition();
    const { latitude: lat, longitude: lng } = pos.coords;

    // Reverse geocoding
    const resGeo = await fetch(
      `https://geocode.xyz/${lat},${lng}?geoit=json&auth=589587661855443523346x111767`
    );
    if (!resGeo.ok) {
...
Promise.allSettled([
  ...
  Promise.resolve('Success'),
]).then(res => console.log(res)); // returns all doesnt matter rejected or fulfuiled
...
Promise.all([
  ...
  .catch(err => console.error(err)); // If one of it rejects whole thing is rejected
...
Promise.any([
  ...
  .then(res => console.log(res))
  .catch(err => console.error(err)); // Ignores rejected ones unless all of them are rejected
